...
CONCURRENTLY_BIN="${CONCURRENTLY_BIN:-./node_modules/.bin/concurrently}"
...
if [[ ! -x "${CONCURRENTLY_BIN}" ]]; then
  echo "concurrently binary not found at ${CONCURRENTLY_BIN}. Run npm install first." >&2
  exit 1
fi
...
echo "Starting Trigger worker and Next dev server on http://localhost:${APP_PORT}"

PROVIDER_VISIBLE_SUPABASE_URL="${TUNNEL_URL}" "${CONCURRENTLY_BIN}" \
  --kill-others \
  --names TRIGGER,AIM_APP \
  --prefix-colors magenta,cyan \
  --timestamp-format "HH:mm:ss" \
  --prefix "[{name}: {time}]" \
  "npm run dev:trigger" \
  "${NEXT_BIN} dev -p ${APP_PORT}"
