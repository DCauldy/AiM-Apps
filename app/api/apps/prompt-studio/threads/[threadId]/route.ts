...
export async function GET(
  ...
  try {
    ...
    const { data: thread, error } = await supabase
      .from("threads")
      .select("id, user_id, title, starred, created_at, updated_at, questions, answers")
      .eq("id", threadId)
      .eq("user_id", user.id)
      .single();
...
export async function PATCH(
  ...
  try {
    ...
    if (Object.keys(updateData).length === 1) {
      return new Response(
        JSON.stringify({ error: "No valid fields to update. Allowed fields: title, starred, questions, answers" }),
        {
          status: 400,
          headers: { "Content-Type": "application/json" },
        }
    ...
    const responseData: any = {
      ...
      questions: updatedThread.questions ?? null,
      answers: updatedThread.answers ?? null,
    };
